...
<nav class="era-timeline" aria-label="Era timeline">
	{#each eras as era (era.id)}
		{@const isExpanded = era.id === selectedEraId}
		<div
			class="era-card"
...
<style>
	...
	.era-icon {
		...
		color: color-mix(in srgb, var(--color-secondary) 120%, white 20%);
		transition: background 0.2s ease, color 0.2s ease;
	}

	.era-card.expanded .era-icon {
		background: color-mix(in srgb, var(--color-secondary) 40%, transparent);
		color: color-mix(in srgb, var(--color-secondary) 150%, white 50%);
	}
